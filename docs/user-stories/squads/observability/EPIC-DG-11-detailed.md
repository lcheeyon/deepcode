...
| **Owning squad / role** | `observability` / `sre` |
| **Phase mapping** | primary `L13`; secondary `C5` |
...
| **Objective** | Verify AC-DG-11-001-01: Callback list includes LangFuse when enabled and LangSmith when enabled (Architecture §8.2). |
...
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-001-01") or Xray/TestRail key == AC-DG-11-001-01 |
...
| **Objective** | Verify AC-DG-11-001-02: Raw LLM output persisted only in tenant PostgreSQL; external sinks capped to 4KiB excerpt (Architecture §23.1 Q9). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-001-02") or Xray/TestRail key == AC-DG-11-001-02 |
...
| **Objective** | Verify AC-DG-11-001-03: `SECRET`-classified fields never exported to traces (Architecture §20.3). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-001-03") or Xray/TestRail key == AC-DG-11-001-03 |
...
| **Objective** | Verify AC-DG-11-002-01: LangFuse observations of type `GENERATION` feed documented aggregation query pattern (Architecture §8.3). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-002-01") or Xray/TestRail key == AC-DG-11-002-01 |
...
| **Objective** | Verify AC-DG-11-002-02: Budget alert triggers when single scan exceeds threshold and can abort scan (Architecture §8.4; job `budget.max_llm_usd` §28.4). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-002-02") or Xray/TestRail key == AC-DG-11-002-02 |
...
| **Objective** | Verify AC-DG-11-003-01: `false-positive-registry` eval cannot regress >2% absolute vs baseline on `main` (Architecture §33.2). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-003-01") or Xray/TestRail key == AC-DG-11-003-01 |
...
| **Objective** | Verify AC-DG-11-003-02: Datasets enumerated in §7.3 exist in `eval/datasets` with documented ownership. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-003-02") or Xray/TestRail key == AC-DG-11-003-02 |
...
| **NFR metric / target** | `p95_latency / Defined in AC statement` |
| **Dependencies** | epics `-`; stories `-`; ACs `-` |
| **Required test layer** | `integration` (min automated tests: `1`) |
| **Framework / control tags** | `-` / `-` |
...
| **Objective** | Verify AC-DG-11-004-01: P95 total scan <10 min for reference ≤500k LOC tracked as SLO (Architecture §21.1, §35.1). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-004-01") or Xray/TestRail key == AC-DG-11-004-01 |
...
| **MoSCoW** | `Should` |
| **Requirement type** | `NFR` |
| **NFR metric / target** | `queue_depth / Defined in AC statement` |
| **Dependencies** | epics `-`; stories `-`; ACs `-` |
| **Required test layer** | `integration` (min automated tests: `1`) |
| **Framework / control tags** | `-` / `-` |
| **Estimate (story points hint)** | `4` |
| **Requirement (verbatim)** | Queue depth >500 pending >15m raises alert (Architecture §35.2). |
| **Objective** | Verify AC-DG-11-004-02: Queue depth >500 pending >15m raises alert (Architecture §35.2). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-004-02") or Xray/TestRail key == AC-DG-11-004-02 |
...
| **Objective** | Verify AC-DG-11-005-01: API emits spans for `POST /v1/scans`, DB queries as child spans where practical (Architecture §26.2, §35). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-005-01") or Xray/TestRail key == AC-DG-11-005-01 |
...
| **Objective** | Verify AC-DG-11-005-02: Worker emits spans per LangGraph node name matching Architecture trace hierarchy §7.1. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-005-02") or Xray/TestRail key == AC-DG-11-005-02 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-005-03: Trace context propagates `scan_id`, `tenant_id` attributes (non-secret). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-005-03") or Xray/TestRail key == AC-DG-11-005-03 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-006-01: Startup or first-use logs resolved prompt revision for `deepguard/athena-*` style keys. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-006-01") or Xray/TestRail key == AC-DG-11-006-01 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-006-02: Offline mode uses bundled prompt pack with version file (EPIC-DG-13). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-006-02") or Xray/TestRail key == AC-DG-11-006-02 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-006-03: Prompt change triggers cache invalidation per §9.4 semantic cache rules. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-006-03") or Xray/TestRail key == AC-DG-11-006-03 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-007-01: Metrics exported: `cache_l1_hit_total`, `semantic_cache_hit_total`, etc. (names documented). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-007-01") or Xray/TestRail key == AC-DG-11-007-01 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-007-02: Per-tenant labels on cost metrics but not on raw code content metrics (privacy). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-007-02") or Xray/TestRail key == AC-DG-11-007-02 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-007-03: Dashboard template JSON checked into `infra/` or `docs/` (optional). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-007-03") or Xray/TestRail key == AC-DG-11-007-03 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-008-01: Retention job deletes or archives traces per tenant policy default 1y with legal hold override. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-008-01") or Xray/TestRail key == AC-DG-11-008-01 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-008-02: Encryption uses app-level AES-256 or pgcrypto as decided in ADR (Architecture §23.1 Q9). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-008-02") or Xray/TestRail key == AC-DG-11-008-02 |
| **Spec references** | Architecture §23.1 Q9 |
...
| **Objective** | Verify AC-DG-11-008-03: Export of raw traces requires `admin` + break-glass logging (EPIC-DG-03). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-008-03") or Xray/TestRail key == AC-DG-11-008-03 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-009-01: When `LANGCHAIN_TRACING_V2=false`, no LangSmith network calls occur (verified by integration test with network deny). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-009-01") or Xray/TestRail key == AC-DG-11-009-01 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-009-02: LangFuse host configurable; missing keys disable handler without error on startup. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. \| 4. Execute negative path and assert stable error_code with no side effects. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-009-02") or Xray/TestRail key == AC-DG-11-009-02 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-009-03: Dual-sink order documented if both enabled (Architecture §8.2). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-009-03") or Xray/TestRail key == AC-DG-11-009-03 |
| **Spec references** | Architecture §8.2 |
...
| **MoSCoW** | `Must` |
| **Requirement type** | `NFR` |
| **NFR metric / target** | `retry_rate / TBD` |
| **Dependencies** | epics `-`; stories `-`; ACs `-` |
| **Required test layer** | `integration` (min automated tests: `1`) |
| **Framework / control tags** | `-` / `-` |
| **Estimate (story points hint)** | `4` |
| **Requirement (verbatim)** | Retry rate computed per scan and aggregated per tenant hourly. |
| **Objective** | Verify AC-DG-11-010-01: Retry rate computed per scan and aggregated per tenant hourly. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-010-01") or Xray/TestRail key == AC-DG-11-010-01 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-010-02: Alert includes top contributing `agent_id` and `error_code` histogram. |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-010-02") or Xray/TestRail key == AC-DG-11-010-02 |
| **Spec references** | Architecture_Design.md (see product spec) |
...
| **Objective** | Verify AC-DG-11-010-03: Silencing / maintenance window supported via standard alertmanager pattern (optional). |
| **Preconditions** | Tenant and scan fixtures exist; services healthy per test environment bootstrap. |
| **Verification procedure** | 1. Arrange test data and configuration to match preconditions. \| 2. Execute the operation or graph/API path under test. \| 3. Assert post-conditions against DB rows, API responses, object store metadata, queue state, or traces. |
| **Expected result** | Assertions pass; no secret material in logs; stable machine-readable error_code on failures; tenant isolation preserved for EPIC-DG-11. |
| **Edge / negative focus** | Concurrent requests do not duplicate side effects; timeout/partial failures reach documented degraded or terminal states. Feature-disabled path yields explicit skip/no-op behaviour. |
...
| **Automation hints** | pytest marker @pytest.mark.req("AC-DG-11-010-03") or Xray/TestRail key == AC-DG-11-010-03 |
| **Spec references** | Architecture_Design.md (see product spec) |


## Epic Definition of Done checklist

- [ ] All in-scope ACs for `EPIC-DG-11` implemented by `observability` and linked to automated tests (`TC-DG-*`).
